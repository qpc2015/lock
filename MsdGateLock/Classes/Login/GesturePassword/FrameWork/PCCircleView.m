...
@property (nonatomic,assign) float circleViewCenterY;
// 选中的圆的集合
...
- (PCCircleView *)initWithcirCleViewCenterY:(CGFloat)centerY
{
    if (self = [super init]) {
        self.circleViewCenterY = centerY;
        // 解锁视图准备
        [self lockViewPrepare];
    }
    return self;
...
-(void)lockViewPrepare{
    ...
    [self setCenter:CGPointMake([UIScreen mainScreen].bounds.size.width/2,kScreenH * self.circleViewCenterY)];
